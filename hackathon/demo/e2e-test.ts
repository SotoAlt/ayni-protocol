 ...
 *
 * Run: npx tsx hackathon/demo/e2e-test.ts
 */
...
async function testServerAPI() {
  ...
  try {
    ...
    console.log('  ⚠ Skipping server tests. Run: cd hackathon/server && npm run dev');
    return;
  }
...
async function main() {
  ...
  if (failed > 0) {
    ...
    console.log('\n✅ All tests passed!');
    console.log('\nNext steps:');
    console.log('  1. Start server: cd hackathon/server && npm run dev');
    console.log('  2. Deploy contracts: forge script script/Deploy.s.sol --rpc-url monad_testnet');
    console.log('  3. Run with server: AYNI_SERVER_URL=http://localhost:3000 npx tsx hackathon/demo/e2e-test.ts');
  }
}
