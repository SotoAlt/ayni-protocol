...
1. Configure OpenClaw's MCP config (`mcp_config.json`):
```json
{
  "servers": {
    "ayni-protocol": {
...
```bash
# From packages/skill directory
clawhub publish SKILL.md
```

**Or reference directly:**

The skill file at [`packages/skill/SKILL.md`](../skill/SKILL.md) can be loaded directly by OpenClaw.
...
- Ensure Ayni server is running: `cd packages/server && npm run dev`
