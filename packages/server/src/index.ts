import './env.js'; // Validate environment first
...
fastify.get('/', async () => {
  return {
    ...
    endpoints: {
      ...
      'POST /stream/broadcast': 'Manually broadcast message',
...
try {
  ...
  console.log(`Ayni server listening on ${env.host}:${env.port} (${env.nodeEnv})`);
} catch (err) {
